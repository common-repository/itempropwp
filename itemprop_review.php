...
class itempropwp_review
{
        ...
        if (is_singular() && !is_feed()) {
            ...
            if (isset($reviewonoff['onoff'])) {
                if ($reviewonoff['onoff'] == "on") {
                    if (!$reviewname) {
                        $reviewname = $reviewpost->post_title;
                    }
                    ...
                    if (!empty($review_descr)) {
                        ...
                        $review_descr = apply_filters(
                                'ipwp_reviewpost_dsc', $ipwrp_n->ipwp_excerpt_maxchr(
                                        get_option('smcipwp_maxlenght'), strip_tags(strip_shortcodes($reviewpost->post_content))
                                )
    ...
    function ipwp_cpbox($post)
    {
        ...
        echo '<td>';
        echo '<div class="' . $ipwprprefix . 'postcbox-input">';
        echo '<input type="text" id="' . $ipwprprefix . 'product_name" name="' . $ipwprprefix . 'product_name" value="' . esc_html(get_post_meta($post->ID, $ipwprprefix . 'product_name', true)) . '" size="25" class="large-text" />';
        echo '</div>';
        echo '</td>';
        echo '</tr>';
        ...
        echo '</div>';
        echo '</th>';
        echo '<td><div class="' . $ipwprprefix . 'postcbox-input"><input type="text" id="' . $ipwprprefix . 'product_price" name="' . $ipwprprefix . 'product_price" value="' . esc_html(get_post_meta($post->ID, $ipwprprefix . 'product_price', true)) . '" size="25" /></div></td>';
        echo '</tr>';
        ...
        echo '</div></th>';
        echo '<td><div class="' . $ipwprprefix . 'postcbox-input"><input type="text" id="' . $ipwprprefix . 'currency" name="' . $ipwprprefix . 'currency" value="' . esc_html(get_post_meta($post->ID, $ipwprprefix . 'currency', true)) . '" size="25" /></div></td></tr>';
